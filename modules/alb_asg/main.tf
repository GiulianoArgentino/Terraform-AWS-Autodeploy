...
# Make SSH key to connect to ec2 instances
...
resource "aws_key_pair" "ec2-keypair" {
  key_name   = "${var.prefix_name}-ec2-key"
  public_key = tls_private_key.ec2-key.public_key_openssh
  
  provisioner "local-exec" {
    command="echo '${tls_private_key.ec2-key.private_key_pem}' > ${var.ssh_key_dir}/${aws_key_pair.ec2-keypair.key_name}.pem"  
  }
}
...
resource "aws_launch_configuration" "my-launchconfig" {
  ...
  security_groups      = [aws_security_group.sg-ssh.id, aws_security_group.sg-instances.id]
